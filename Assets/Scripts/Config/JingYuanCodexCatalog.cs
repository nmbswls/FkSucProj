...
namespace My.Config
{
    public static class JingYuanCodexCatalog
    {
        ...
        public static JingYuanTuneTier ResolveTuneTier(string codexId, long concentration, int codexLevel)
        {
            if (CfgMgr.Cfgs == null || string.IsNullOrEmpty(codexId))
            {
                return null;
            ...
            JingYuanTuneTier best = null;
            foreach (var row in CfgMgr.Cfgs.TbJingYuanTuneTier.DataList)
            {
                if (row == null || row.CodexId != codexId)
                {
                    continue;
                ...
                if (codexLevel < row.MinCodexLevel)
                {
                    continue;
                }

                if (concentration < row.NeedConcentration)
                {
                    continue;
                }

                if (best == null || row.Tier > best.Tier)
                {
                    best = row;
                }
            }
        ...
        public static int GetConcentrationLevel(long concentration)
        {
            var cfgs = CfgMgr.Cfgs?.TbTiaoJingConcentrationLevel?.DataList;
            if (cfgs == null)
            {
                return 0;
            }
            ...
            for (int i = 0; i < cfgs.Count; i++)
            {
                ...
                if (concentration < row.NeedAmount)
                {
                    break;
                }
        ...
        public static float GetConcentrationCostPerSec(int concentrationLevel)
        {
            var row = CfgMgr.Cfgs?.TbTiaoJingConcentrationLevel?.GetOrDefault(concentrationLevel);
            return row?.CostPerSec ?? 0f;
        }
