...
namespace My.MiniGame.Dream
{
    ...
    {
        [Header("Prefab layout（可空则按 Box/Body、Box/CloseBtn 解析 / 再退回代码生成）")]
        ...
        private void ResolveLayoutOrBuild()
        {
            ...
            var bodyTr = _rootRt.Find("Box/Body");
            _bodyTmp = bodyText != null ? bodyText : bodyTr != null ? bodyTr.GetComponent<TextMeshProUGUI>() : null;
            var closeTr = _rootRt.Find("Box/CloseBtn");
            _closeBtn = closeButton != null ? closeButton : closeTr != null ? closeTr.GetComponent<Button>() : null;

            if (_bodyTmp != null && _closeBtn != null)
            {
                _layoutBuilt = true;
        ...
        private void BuildIfNeeded()
        {
            ...
            _closeBtn = btnGo.GetComponent<Button>();
...

        {
            ...
            if (_bodyTmp == null) ResolveLayoutOrBuild();

            if (_bodyTmp == null)

            {

                Debug.LogWarning("[DreamInfiltration] Settlement body text missing.");
