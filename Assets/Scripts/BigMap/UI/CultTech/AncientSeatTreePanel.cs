...
namespace My.UI.CultTech
{
    public sealed class AncientSeatTreePanel : PanelBase
    {
        ...
        void Awake() { panelId = Pid; treeView ??= GetComponentInChildren<AncientSeatTreeView>(true); }
        public override void Setup(object data = null) { base.Setup(data); treeView?.Bind(ResolveCult()); }
        public override void Show() { base.Show(); treeView?.Refresh(); }
